...
export default class Game implements GameConfig {
	...
	move(cube: Cube, startPad: Pad, endPad: Pad) {
		...
		this.pads.forEach((pad) => {
			if (startPad.position < pad.position && endPad.position > pad.position) {
				cube.onPassingBy && cube.onPassingBy(pad, endPad, this.data);
				pad.stack.forEach((cubeOnPassedByPad) => {
					cubeOnPassedByPad.onPassedBy &&
						cubeOnPassedByPad.onPassedBy(endPad, pad, this.data);
				});
			}
		});

		console.log(
			`${movedCubes.map((c) => c.name).join(', ')} moved from (${
				startPad.position
			}) to (${endPad.position})`
		);
	}
	...
	nextRound() {
		...
		turnOrders.forEach((cube) => {
			...
			if (nextPad.stack.length > 1) {
				nextPad.stack.forEach((cubeInStack) => {
					cubeInStack.onStacked && cubeInStack.onStacked(nextPad, this.data);
				});
			}
